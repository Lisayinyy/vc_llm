...
Object.entries(DATA.shareholders_by_company).forEach(([coShort, shs]) => {
  shs.forEach((sh, idx) => {
    ...
    edges.push({
      ...
      // weight: 反映持股比例. 上限 2.5 (避免 50%+ 边过粗)
      weight: typeof sh.ratio === 'number' ? Math.min(2.5, Math.max(0.5, sh.ratio / 5)) : 0.5,
      ratio: sh.ratio,
      round: sh.round || '',
...
const investorRank = Object.entries(investorAppearance)
  ...
  .slice(0, 12);  // v9.2: 改成 12, 展示所有跨公司机构方
...
const verifiedInvestorSuggestions = [...new Set(
  ...
  return { id: 'sh:' + name, name, cat: 'investor', count, label: `已核实关联 ${count} 家` };
...
const suggestions = [
  ...
  // v9.2: 不再在中心选择器里提供个人 GP 按钮 (个人隶属于机构)
];
